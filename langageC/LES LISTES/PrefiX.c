...
eliste *p1,*p2;
 ...
 prefix=true;
if(p1!=NULL){
        while(p2!=NULL && p1!=NULL && prefix==true){
         if(p1->val!=p2->val){
            prefix=false;};
...
int n,g;
eliste *T1,*T2;
printf("-> donner le nombre d'elements a incerer pour la premiere liste L1( attention c'est une creation LIFO )\n");
scanf("%d",&n);
...
affichage(T2);
printf("\n->Resultat: %s",estPrefixe(T1,T2)? "true" : "false");
